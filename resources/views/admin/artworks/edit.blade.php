        ...
        <div class="bg-white dark:bg-ash-900 p-8 rounded-lg shadow border border-gray-100 dark:border-ash-800">
            <form action="{{ route('admin.artworks.update', $artwork) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="flex items-start gap-4">
                    <img src="{{ asset('images/' . $artwork->image) }}" class="w-24 h-24 object-cover rounded border border-gray-200 dark:border-ash-700">
                    <div class="flex-1">
                        <x-label for="image" value="Replace Image (Keep empty to preserve current)" />
                        <input id="image" type="file" name="image" accept="image/*" class="mt-1 block w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-black file:text-white hover:file:bg-gray-800 dark:file:bg-white dark:file:text-black dark:hover:file:bg-gray-200 transition" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="5" class="border-gray-300 dark:border-ash-700 focus:border-black dark:focus:border-white focus:ring-black dark:focus:ring-white rounded-md shadow-sm block mt-1 w-full bg-white dark:bg-black text-gray-900 dark:text-white" required>{{ $artwork->description }}</textarea>
                </div>
